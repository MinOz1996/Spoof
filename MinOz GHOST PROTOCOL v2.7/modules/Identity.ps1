...
function Apply-AegisIdentity {
    ...
    try {
        ...
        # 2b. PERMANENT BIOS Keys (Critical for VIEW to show [SUCCESS])
        Write-AegisLog -Level "INFO" -Message "[MinOz] Writing PERMANENT BIOS keys..."
        ...
        try {
            if (-not (Test-Path $RegBiosPermanent)) {
                ...
                Write-AegisLog -Level "DEBUG" -Message "[MinOz] Created SystemInformation key"
            }
            
            Set-ItemProperty -Path $RegBiosPermanent -Name "SystemManufacturer" -Value $Identity.Manufacturer -Force -ErrorAction Stop
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set SystemManufacturer: $($Identity.Manufacturer)"
            
            Set-ItemProperty -Path $RegBiosPermanent -Name "SystemProductName" -Value $Identity.Product -Force -ErrorAction Stop
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set SystemProductName: $($Identity.Product)"
            
            Set-ItemProperty -Path $RegBiosPermanent -Name "BIOSVendor" -Value $Identity.BiosVendor -Force -ErrorAction Stop
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set BIOSVendor: $($Identity.BiosVendor)"
            
            Set-ItemProperty -Path $RegBiosPermanent -Name "BIOSVersion" -Value $Identity.BiosVersion -Force -ErrorAction Stop
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set BIOSVersion: $($Identity.BiosVersion)"
            
            Set-ItemProperty -Path $RegBiosPermanent -Name "BIOSReleaseDate" -Value $Identity.BiosDate -Force -ErrorAction Stop
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set BIOSReleaseDate: $($Identity.BiosDate)"
            
            Write-AegisLog -Level "INFO" -Message "[MinOz] PERMANENT BIOS keys written successfully"
        }
        catch {
            Write-AegisLog -Level "ERROR" -Message "[MinOz] Failed to write PERMANENT BIOS keys: $($_.Exception.Message)"
        }
        
        # 2c. CPU Masking (PERMANENT)
        Write-AegisLog -Level "INFO" -Message "[MinOz] Masking CPU..."
        try {
            if (Test-Path $RegCpu) {
                Set-ItemProperty -Path $RegCpu -Name "ProcessorNameString" -Value $Identity.CPU -Force -ErrorAction Stop
                Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set CPU: $($Identity.CPU)"
            }
        }
        catch {
            Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to mask CPU: $($_.Exception.Message)"
        }
        
        # 2d. GPU Masking (Enum keys)
        Write-AegisLog -Level "INFO" -Message "[MinOz] Masking GPU..."
        $gpuCount = 0
        try {
            if (Test-Path $RegDisplayEnum) {
                Get-ChildItem $RegDisplayEnum -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                    try {
                        Set-ItemProperty -Path $_.PSPath -Name "DeviceDesc" -Value $Identity.GPU -Force -ErrorAction Stop
                        Set-ItemProperty -Path $_.PSPath -Name "FriendlyName" -Value $Identity.GPU -Force -ErrorAction Stop
                        $gpuCount++
                    } catch {}
                }
                Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set GPU on $gpuCount devices: $($Identity.GPU)"
            }
        }
        catch {
            Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to mask GPU: $($_.Exception.Message)"
        }
